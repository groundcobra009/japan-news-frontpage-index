...
記事全文は保存しない方針のため、当日収集した見出しのみを入力とする。本モジュールは
外部AI APIに依存せず、janome(Pure Python形態素解析)による名詞頻度ベースの
ヒューリスティックで実装する(「本日のまとめ」のClaude API生成が失敗した場合の
決定的なフォールバック元としても使われるため、この非依存性を維持すること)。
"""
